...
public class Camera {
    ...
    private final Vector3f targetPosition = new Vector3f(position);
    ...
    public void setPosition(Vector3f position) {
        // 1. Set the instantaneous position
        this.position.set(
                position.x,
                position.y,
                position.z
        );
        // 2. SYNCHRONIZE the target position to prevent unwanted LERP movement in the next update
        this.targetPosition.set(this.position);
    }
    
    public void setPosition(float x, float y, float z) {
        this.position.set(x, y, z);
        // SYNCHRONIZE the target position
        this.targetPosition.set(this.position);
    }
